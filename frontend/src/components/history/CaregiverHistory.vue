...
const TREND_RANGE_POLICY: Record<TrendRangeKey, { rangeDays: number; bucketSize: number; pointCount: number }> = {
  ...
  "3m": { rangeDays: 91, bucketSize: 7, pointCount: 13 },
  "6m": { rangeDays: 182, bucketSize: 7, pointCount: 26 },
};
...
const chartCanvasWidth = computed(() => {
  if (selectedRange.value !== "6m") return 700;
  const minPointGap = 48;
  const horizontalPadding = 32;
  const points = Math.max(2, trendValues.value.length);
  return horizontalPadding + (points - 1) * minPointGap;
});
      ...
      <section class="card">
        ...
        <div class="trend-chart-viewport">
          <div
            ...
            :style="{ width: selectedRange === '6m' ? `${chartCanvasWidth}px` : '100%' }"
          >
            <WeeklyChart
              :data="trendValues"
              :states="trendStates"
              ...
              :chart-width-px="chartCanvasWidth"
              :padding-left-px="selectedRange === '6m' ? 16 : 66"
              :padding-right-px="selectedRange === '6m' ? 16 : 28"
